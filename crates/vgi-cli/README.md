...
- **GitHub**, through your `gh` login (`gh auth login`; admin on the
  ...
  no deletion, no bypass actors). With two or more owners — a personal
  repository's account holder, plus each `--code-owner <login>` — it also
  writes the managed `.github/CODEOWNERS` block and requires a code owner's
  review; with one, only the check.
- **Forgejo / Gitea**, with `FORGEJO_TOKEN` (`write:repository`,
  `read:user`). It allows fast-forward-only merges, commits
...
referral (`--registry`); the verify-trust action pinned to the commit of the
release matching this binary (`--verify-trust-action`,
`--verify-trust-version`); on Forgejo, the release's published tarball
SHA-256 (`--verify-trust-sha256` to pin one you verified); on github.com, the
`web-flow` key (`--platform-keyring`).
